...
const AdminLayout = () => {
  ...
  // Helper function to determine if a link is active based on pathname
    ...
    <div className="flex min-h-screen bg-gradient-to-br from-yellow-50 to-amber-50">
      ...
      <div className="fixed top-0 left-0 right-0 h-12 sm:h-16 bg-gradient-to-r from-yellow-500 to-amber-600 border-b border-yellow-400 z-50 flex items-center justify-between px-2 sm:px-4 shadow-lg">
        <div className="flex items-center space-x-2 sm:space-x-3">
          ...
          <h1 className="text-sm sm:text-xl font-bold text-gray-800">Admin Panel</h1>
        </div>
        <div className="flex items-center space-x-2 sm:space-x-4">
          <div className="hidden sm:block text-sm text-gray-700">Logged in as Admin</div>
          <button
            onClick={handleLogout}
            className="bg-red-500 hover:bg-red-600 text-white font-semibold py-1.5 px-2.5 sm:py-2 sm:px-4 text-xs sm:text-sm rounded-lg transition duration-300"
          >
            Logout
          </button>
        </div>
          ...
          <ul>
            ...
            <li className="mt-2">
              <Link
                ...
                className={getNavLinkClass("/admin/resellers")}
                onClick={() => setSidebarOpen(false)}
              >
                <ResellerIcon />
                <span className="text-sm sm:text-base">Reseller</span>
              </Link>
            </li>
          </ul>
        </nav>
      ...
      >
        {/* Manual Routing based on pathname, replacing <Outlet /> */}
        {pathname === "/admin" && <AdminDashboardPage />}
        {pathname === "/admin/products" && <AdminProductsPage />}
        {pathname === "/admin/products/new" && <AdminProductFormPage />}
        {pathname.startsWith("/admin/products/") &&
          pathname.includes("/edit/") && <AdminProductFormPage />}
        {pathname === "/admin/transactions" && <AdminTransactionsPage />}
        {pathname === "/admin/resellers" && <AdminResellersPage />}
        {pathname === "/admin/resellers/new" && <AdminResellerFormPage />}
        {pathname.startsWith("/admin/resellers/") &&
          pathname.includes("/edit/") && <AdminResellerFormPage />}
      </main>
    </div>
  );
};
