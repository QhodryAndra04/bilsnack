...
const AdminTransactionsPage = () => {
  ...
  const submitTracking = async () => {
    ...
    try {
      ...
      showNotification("Berhasil", "Tracking berhasil diperbarui!", "success");
      setShowModal(false);
      // Refresh data
      loadTransactions();
    } catch (error) {
  ...
  const updateStatus = async (id, newStatus) => {
    try {
      ...
      showNotification("Berhasil", `Status transaksi #${id} berhasil diubah menjadi ${newStatus}`, "success");
    } catch (error) {
      alert(`Error: ${error.message}`);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-6">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-100 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
          ...
          <p className="text-xl font-bold mt-2 text-purple-600 overflow-wrap-anywhere">
            {formatPrice(
              transactions
        ...
        <div className="flex flex-wrap gap-4">
          <div className="w-full sm:w-auto">
            ...
            <select
              ...
              className="w-full sm:w-48 border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-yellow-500 text-gray-700 bg-white"
            >
          ...
          <table className="w-full text-left table-auto">
            <thead>
              <tr className="bg-yellow-500 text-white">
                ...
                <th className="p-4 font-semibold whitespace-nowrap">ID Pesanan</th>
                <th className="p-4 font-semibold whitespace-nowrap">
                  Pelanggan
                </th>
                <th className="p-4 font-semibold whitespace-nowrap">Role</th>
                <th className="p-4 font-semibold whitespace-nowrap">Jumlah</th>
                ...
                <th className="p-4 font-semibold whitespace-nowrap">Aksi</th>
              </tr>
            </thead>
            <tbody>
                  ...
                  <tr key={t.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="p-4">
                      <select
                        ...
                        className={`text-xs font-semibold px-2 py-1 rounded-full border-0 cursor-pointer outline-none ${getStatusColor(
                          t.status
                        )}`}
                      >
                        <option value="Menunggu">Menunggu</option>
                    ...
                    <td className="p-4">
                      <button
                        ...
                        className="bg-blue-50 hover:bg-blue-100 text-blue-600 px-3 py-1.5 rounded-md text-xs font-medium transition-colors flex items-center gap-1"
                      >
                        <svg
                          className="w-3 h-3"
                          fill="none"
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black/50 z-50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-2xl border border-gray-100">
            <h3 className="text-lg font-bold text-gray-900 mb-4">
              Set Tracking Order #{selectedOrder.id}
            </h3>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Catatan / History (JSON)
                </label>
                <textarea
                  rows={3}
                  value={trackingHistoryText}
                  onChange={(e) => setTrackingHistoryText(e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:outline-none text-xs font-mono"
                  placeholder='[{"status": "shipped", "time": "..."}]'
                />
              </div>
            </div>
            ...
            <div className="flex justify-end gap-3 mt-6">
              ...
              <button
                ...
                className="px-4 py-2 text-white bg-green-600 hover:bg-green-700 rounded-lg text-sm font-medium transition-colors shadow-sm"
              >
                Simpan
              </button>
            </div>
